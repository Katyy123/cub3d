...
t_bool increment_d(float *d, t_game *game, float ray_a)
{
    ...
    float   ipx; //distanza lungo x dalla casella che sto controllando
    float   ipy; //distanza lungo y dalla casella che sto controllando

    float m = tan(ray_a);  //tangente o coefficente angolare
    float sx = sqrt(1 + m*m); //passo sulla x che corrisponde al passo di una casella sulla y
    float sy = sqrt(1 + (1/m)*(1/m)); //passo sulla y che corrisponde al passo di una casella sulla x
...
// prox. casella
    ...
    {
        stepx = -1;
        ipx = (game->pl.pos_x - (float)eye_x) * sx;
    }
    else
    {
        stepx = 1;
        ipx = (((float)eye_x + 1) - game->pl.pos_x) * sx;
    }
    if (sin_a < 0)
    {
        stepy = -1;
        ipy = (game->pl.pos_y - (float)eye_y) * sy;
    }
    else
    {
        stepy = 1;
        ipy = (((float)eye_y + 1) - game->pl.pos_y) * sy;
    }
    ...
    // while che fa tutti i controlli:
